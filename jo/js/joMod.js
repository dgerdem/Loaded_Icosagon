...
if (typeof HTMLElement === 'undefined') HTMLElement = Object;

// no console.log? sad...
if (typeof console === 'undefined' || typeof console.log !== 'function') console = {
    log: function (msg) {}
};
...
jo = {
    ...
    useragent: ['ipad', 'iphone', 'playbook', 'webos', 'bada', 'android', 'opera', 'chrome', 'safari', 'mozilla', 'gecko', 'explorer'],
    ...
    load: function (call, context) {
        ...
        if (typeof document.body.style.webkitTransition !== "undefined") {
            ...
            s.transitionEnd = "transitionend";
            s.setPosition = function (x, y, node) {
                if (this.vertical) node.style.top = y + "px";

                if (this.horizontal) node.style.left = x + "px";
            };
        }
    ...
    initTagMap: function () {
        ...
        for (var p in window) {
            var o = window[p];
            if (typeof o === 'function' && o.prototype && typeof o.prototype.tagName !== 'undefined' && o.prototype instanceof joView) {
...
joDOM = {
    ...
    get: function (id) {
        if (typeof id === "string") {
            ...
            if (id instanceof joView) return id.container;
            else return id;
        }
    },
    ...
    create: function (tag, style) {
        ...
        if (typeof tag === "object" && typeof tag.tagName === "string") {
            ...
            if (tag.className) this.setStyle(o, tag.className);
        } else {
            o = document.createElement(tag);

            if (style) this.setStyle(o, style);
        }
    ...
    loadCSS: function (filename, oldnode) {
        ...
        if (!oldnode) document.body.appendChild(css);
    ...
    pageOffsetLeft: function (node) {
        ...
        while (typeof node !== 'undefined' && node && node.parentNode !== window) {
            if (node.offsetLeft) l += node.offsetLeft;
...
joEvent = {
    ...
    on: function (element, event, call, context, data, capture) {
        if (!call || !element) return false;

        if (this.touchy) {
            if (this.eventMap[event]) event = this.eventMap[event];
        }
        ...
        function wrappercall(e) {
            ...
            if (context) call.call(context, e, data);
            else call(e, data);
        }
        ...
        if (!window.addEventListener) element.attachEvent("on" + event, wrappercall);
        else element.addEventListener(event, wrappercall, capture || false);
    ...
    remove: function (element, event, call, capture) {
        ...
        if (typeof element.removeEventListener !== 'undefined') element.removeEventListener(event, call, capture || false);
    },
...
joSubject.prototype = {
    ...
    resume: function (data) {
        if (this.last != -1) this.fire(data, true);
    ...
    fire: function (data, resume) {
        ...
        for (var l = this.subscriptions.length; i < l; i++) {
            ...
            if (sub.observer) sub.call.call(sub.observer, data, this.subject, subjectdata);
            else sub.call(data, this.subject, subjectdata);
...
joCache = {
    ...
    get: function (key) {
        ...
        if (cache) {
            if (!cache.view) cache.view = cache.call.apply(cache.context, arguments);
...
joChain.prototype = {
    ...
    next: function () {
        ...
        if (this.queue.length) this.timer = joEvent.yield(this.next, this, this.delay);
...
joDataSource.prototype = {
    ...
    setData: function (data) {
        ...
        if (data !== last) this.changeEvent.fire(data);
    ...
    getPageCount: function () {
        if (this.pageSize) return Math.floor(this.getData().length / this.pageSize) + 1;
        else return 1;
    },
    ...
    getPage: function (index) {
        ...
        if (end > this.getData().length) end = this.getData().length;
...
joRecord.extend(joDataSource, {
    ...
    getDelegate: function (p) {
        if (typeof this.data[p] === "undefined") this.data[p] === null;

        if (!this.delegate[p]) this.delegate[p] = new joProperty(this, p);
    ...
    setProperty: function (p, data) {
        if (typeof this.data[p] !== 'undefined' && this.data[p] === data) return;
...
joProperty.extend(joDataSource, {
    setData: function (data) {
        if (this.datasource) this.datasource.setProperty(this.p, data);
...
joDatabase.prototype = {
    ...
    now: function (offset) {
        ...
        if (offset) date.setDate(date.valueOf() + (offset * 1000 * 60 * 60 * 24));
...
joFileSource.extend(joDataSource, {
    ...
    callBack: function (data, error) {
        if (error) this.errorEvent.fire(error);
...
joFile = function (url, call, context, timeout) {
    ...
    if (!req) return onerror();

    // 30 second default on requests
    if (!timeout) timeout = 60 * SEC;
    ...
    function onchange(e) {
        ...
        if (req.readyState == 4) handler(req.responseText, 0);
    }
    ...
    function handler(data, error) {
        if (call) {
            if (context) call.call(context, data, error);
            else call(error, data, error);
        }
    }
};
...
function joScript(url, call, context) {
    ...
    function handler(error) {
        if (call) {
            if (context) call.call(context, error, url);
            else call(error, url);
        }
...
joYQL.extend(joDataSource, {
    ...
    load: function (data) {
        ...
        if (!results) this.errorEvent.fire(data);
        else {
            this.data = results;
...
joDepot = function (call, context) {
    ...
    function handler(data) {
        if (context) call.call(context, data);
        else call(data);
    }
...
joDispatch.prototype = {
    load: function (url) {
        ...
        if (!h) {
            ...
            if (h.context) this.loadEvent.fire(h.call.call(h.context, p, url));
            else this.loadEvent.fire(h.call(p, url));
        }
    ...
    getHandler: function (url) {
        ...
        for (var i = 0, l = h.length; i < l; i++) {
            // console.log(h[i].url);
            if (url.indexOf(h[i].url, 0) === 0) return h[i];
        }
    ...
    get: function (parent) {
        ...
        function newview(node, args) {
            ...
            if (jo.tagMap[tag]) {
                ...
                if (args instanceof Text) args = node.nodeData;

                if (!args) args = node.value || node.checked || node.innerText || node.innerHTML;
                ...
                if (typeof o === "function") view = new o(args);
                else joLog("joInterface can't process ", tag, "'type' attribute?");
            }

            // keep track of named controls
            if (node.id) ui[node.id] = view;
...
joView.prototype = {
    ...
    refresh: function () {
        if (!this.container || typeof this.data === "undefined") return this;
    ...
    detach: function (parent) {
        ...
        if (this.container && this.container.parentNode === node) node.removeChild(this.container);
...
joControl = function (data, value) {
    ...
    if (typeof value !== "undefined" && value !== null) {
        if (value instanceof joDataSource) this.setValueSource(value);
...
joControl.extend(joView, {
    ...
    setReadOnly: function (value) {
        if (typeof value === 'undefined' || value) this.container.setAttribute('readonly', '1');
        else this.container.removeAttribute('readonly');
    ...
    onFocus: function (e) {
        ...
        if (this.enabled) joFocus.set(this);
    },
...
joList.extend(joControl, {
    ...
    setDefault: function (msg) {
        ...
        if (typeof this.data === 'undefined' || !this.data || !this.data.length) {
            if (typeof msg === 'object') {
                this.innerHTML = "";
                if (msg instanceof joView) this.container.appendChild(msg.container);
                else if (msg instanceof HTMLElement) this.container.appendChild(msg);
            } else {
                this.innerHTML = msg;
            }
        }
    ...
    draw: function () {
        ...
        if (typeof this.data === 'undefined' || !this.data || !this.data.length) {
            if (this.defaultMessage) this.container.innerHTML = this.defaultMessage;
        ...
        for (var i = 0, l = this.data.length; i < l; i++) {
            ...
            if (typeof element === "string") html += element;
            else this.container.appendChild((element instanceof joView) ? element.container : element);
        ...
        if (html.length) this.container.innerHTML = html;

        // refresh our current selection
        if (this.value >= 0) this.setValue(this.value, true);

        return;
    },

    deselect: function () {
        if (typeof this.container == 'undefined' || !this.container['childNodes']) return;
    ...
    setValue: function (index, silent) {
        ...
        if (typeof this.container === 'undefined' || !this.container || !this.container.firstChild) {
            return this;
        }

        var node = this.getNode(this.value);
        if (node) {
            if (this.lastNode) joDOM.removeCSSClass(this.lastNode, "selected");
    ...
    onMouseDown: function (e) {
        ...
        if (index >= 0) this.setValue(index);
    },
    ...
    getNodeData: function (index) {
        if (this.data && this.data.length && index >= 0 && index < this.data.length) return this.data[index];
        else return null;
    },
    ...
    getNodeIndex: function (element) {
        ...
        if (typeof index !== "undefined" && index !== null) return parseInt(index);
        else return -1;
    },
    ...
    next: function () {
        if (this.getValue() < this.getLength() - 1) this.setValue(this.value + 1);

        return this;
    },

    prev: function () {
        if (this.getValue() > 0) this.setValue(this.value - 1);
...
joStack = function (data) {
    ...
    if (this.container && this.container.firstChild) this.container.innerHTML = "";
...
joStack.extend(joContainer, {
    ...
    draw: function () {
        ...
        function animate() {
            ...
            // better
            if (typeof window.onwebkittransitionend !== 'undefined') transitionevent = joEvent.on(newchild, "webkitTransitionEnd", cleanup, self);
            else joDefer(cleanup, this, 200);

            if (newclass && newchild) joDOM.removeCSSClass(newchild, newclass);

            if (oldclass && oldchild) joDOM.addCSSClass(oldchild, oldclass);
        }
        ...
        function cleanup() {
            ...
            if (newchild) {
                if (transitionevent) joEvent.remove(newchild, "webkitTransitionEnd", transitionevent);
        ...
        if (typeof this.data[this.index].activate !== "undefined") this.data[this.index].activate.call(this.data[this.index]);
    ...
    removeChild: function (child) {
        if (child && child.parentNode === this.container) this.container.removeChild(child);
    },
    ...
    push: function (o) {
        ...
        if (this.data && this.data.length && this.data[this.data.length - 1] === o) return this;
    ...
    pop: function () {
        if (this.data.length > this.locked) {
            ...
            if (typeof o.deactivate === "function") o.deactivate.call(o);

            if (!this.data.length) this.hide();
        }

        if (this.data.length > 0) this.popEvent.fire();
...
joScroller.extend(joContainer, {
    ...
    onMove: function (e) {
        ...
        if (this.points.length > 7) this.points.pop();
        ...
        this.timer = window.setTimeout(function () {
            if (self.inMotion && self.points.length > 1) self.points.pop();

            self.timer = null;
        }, this.interval);

        if (this.moved) this.scrollBy(x, y, true);

        if (!this.moved && this.points.length > 3) this.moved = true;
    },
    ...
    onUp: function (e) {
        ...
        if ((Math.abs(dy) * this.vertical > 4 || Math.abs(dx) * this.horizontal > 4)) {
            ...
            // the top or bottom
            if ((flick + top < max || flick + top > 0) || (flickx + left < maxx || flickx + left > 0)) {
    ...
    scrollBy: function (x, y, test) {
        ...
        if (this.vertical && (node.offsetHeight <= this.container.offsetHeight)) return this;
        ...
        if (this.bumpHeight) {
            if (dy > this.bumpHeight) dy = this.bumpHeight;
            else if (dy < max - this.bumpHeight) dy = max - this.bumpHeight;
        }

        if (this.bumpWidth) {
            if (dx > this.bumpWidth) dx = this.bumpWidth;
            else if (dy < maxx - this.bumpWidth) dx = maxx - this.bumpWidth;
        }

        if (!this.eventset) this.eventset = joEvent.capture(node, this.transitionEnd, this.snapBack, this);

        if (top != dx || left != dy) this.moveTo(dx, dy);
    ...
    scrollTo: function (y, instant) {
        ...
        if (typeof y === 'object') {
            var e;
            if (y instanceof HTMLElement) e = y;
            else if (y instanceof joView) e = y.container;
            ...
            if (t - h < bottom) y = (t - h) + this.container.offsetHeight;

            if (y < t) y = t;
        }

        if (y < 0 - node.offsetHeight) y = 0 - node.offsetHeight;
        else if (y > 0) y = 0;
    ...
    snapBack: function () {
        ...
        if (this.eventset) joEvent.remove(node, this.transitionEnd, this.eventset);
...
joExpando.extend(joContainer, {
    ...
    toggle: function () {
        if (this.container.className.indexOf("open") >= 0) return this.close();
...
joExpandoTitle.extend(joControl, {
    ...
    draw: function () {
        if (this.data && this.container) this.container.innerHTML = this.data + "<joicon></joicon>";
    }
});
/**
 joFlexrow
...
joFocus = {
    ...
    set: function (control) {
        if (this.last && this.last !== control) this.last.blur();
...
joHTML.extend(joControl, {
    ...
    onClick: function (e) {
        ...
        function findhref(node) {
            ...
            if (typeof node.parentNode !== "undefined" && node.parentNode !== container) return findhref(node.parentNode);
            else return null;
        }
    }
});
...
joInput.extend(joControl, {
    ...
    setData: function (data) {
        if (data !== this.data) {
            ...
            if (typeof this.container.value !== "undefined") this.container.value = data;
            else this.container.innerHTML = data;
    ...
    getData: function () {
        if (typeof this.container.value !== "undefined") return this.container.value;
        else return this.container.innerHTML;
    },
...
*/
    ...
    draw: function () {
        if (this.container.value) this.value = this.data;
        else this.innerHTML = this.value;
    },
    ...
    storeData: function () {
        ...
        if (this.dataSource) this.dataSource.set(this.value);
    }
});
...
joMenu.extend(joList, {
    ...
    fireSelect: function (index) {
        if (typeof this.data[index].id !== "undefined" && this.data[index].id) this.selectEvent.fire(this.data[index].id);
        else this.selectEvent.fire(index);
    },
...
joScreen.extend(joContainer, {
    ...
    showPopup: function (data) {
        ...
        if (!this.popup) {
            ...
            new joFlexcol(['&nbsp', this.popup = new joPopup(data), '&nbsp']));
        } else {
    ...
    alert: function (title, msg, options, context) {
        ...
        var view = [
        new joTitle(title), new joHTML(msg), buttons];
        this.showPopup(view);
        ...
        function addbutton(options) {
            ...
            function () {
                if (options.action) options.action.call(options.context);
        ...
        function defaultaction() {
            self.hidePopup();
            if (callback) {
                if (context) callback.call(context);
                else callback();
            }
        }
...
joShim.extend(joContainer, {
    ...
    show: function () {
        ...
        if (!this.lastParent) this.lastParent = document.body;
...
joSound.prototype = {
    play: function () {
        if (!this.audio || this.audio.volume === 0) return;
    ...
    onEnded: function (e) {
        ...
        if (++this.repeat < this.repeatCount) this.play();
        else this.repeat = 0;
    },
    ...
    setVolume: function (vol) {
        if (!this.audio || vol < 0 || vol > 1) return;
...
joStackScroller.extend(joStack, {
    ...
    forward: function () {
        if (this.index < this.data.length - 1) this.switchScroller();
    ...
    back: function () {
        if (this.index > 0) this.switchScroller();
    ...
    push: function (o) {
        // don't push the same view we already have
        if (this.data && this.data.length && this.data[this.data.length - 1] === o) return;
    ...
    pop: function () {
        if (this.data.length > this.locked) this.switchScroller();
...
joTabBar.extend(joList, {
    ...
    formatItem: function (data, index) {
        ...
        if (data.label) o.innerHTML = data.label;

        if (data.type) o.className = data.type;
...
joTable.extend(joList, {
    ...
    getRow: function (index) {
        if (typeof index === "undefined") index = this.getIndex();
...
joDialog = function (data) {
    joShim.call(this, new joFlexcol(['', new joPopup(
    new joScroller(data)).setStyle("show"), '']));
};
...
joNavbar = function (title) {
    ...
    var ui = [
    this.titlebar = new joView(title || '&nbsp;').setStyle('title'), new joFlexrow([this.back = new joBackButton('Back').selectEvent.subscribe(this.back, this), ""])];
...
joNavbar.extend(joContainer, {
    ...
    update: function () {
        ...
        if (this.stack.data.length > 1) joDOM.addCSSClass(this.back, 'active');
        else joDOM.removeCSSClass(this.back, 'active');

        var title = this.stack.getTitle();

        if (typeof title === 'string') this.titlebar.setData(title);
        else this.titlebar.setData(this.firstTitle);
...
joSelect = function (data, value) {
    var v = value;
    if (value instanceof joDataSource) v = value.getData();

    var ui = [
    this.field = new joSelectTitle(v), this.list = new joSelectList(data, value)];
...
joSelectTitle.extend(joExpandoTitle, {
    ...
    setData: function (value) {
        if (this.list) joExpandoTitle.prototype.setData.call(this, this.list.getNodeData(value) || "Select...");
        else joExpandoTitle.prototype.setData.call(this, value);
...
joToggle.extend(joControl, {
    ...
    setLabels: function (labels) {
        if (labels instanceof Array) this.labels = labels;
        else if (arguments.length == 2) this.labels = arguments;
    ...
    draw: function () {
        ...
        if (this.data) joDOM.addCSSClass(this.container, "on");
        else joDOM.removeCSSClass(this.container, "on");
    }
});
/**
 joSlider
...
joSlider.extend(joControl, {
    ...
    setRange: function (min, max, snap) {
        ...
        if (min < 0 && max >= 0) this.range = Math.abs(min) + max;
        else if (min < 0 && max <= 0) this.range = min - max;
        else this.range = max - min;

        if (typeof snap !== 'undefined') this.snap = (snap >= 0 && snap <= this.range) ? snap : 0;
        else this.snap = 0;
    ...
    adjustValue: function (v) {
        ...
        if (this.snap) value = Math.floor(value / this.snap) * this.snap;
    ...
    onClick: function (e) {
        if (this.inMotion || this.moved) return;
